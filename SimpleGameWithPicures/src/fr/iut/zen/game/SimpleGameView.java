...
public record SimpleGameView(int xOrigin, int yOrigin, int length, int width, int squareSize) implements GameView {
	...
	public void drawGameInfos(Graphics2D graphics, SimpleGameData data) {
		...
		graphics.drawString("     "+data.getLoopCount(), width+205, 60);
		
		graphics.setColor(Color.white);
		drawImageByPixel(graphics,  width+265,25, "pictures/days.png");
		graphics.drawString("       "+(int) TimeData.getDay(), width+270, 60);
		
		graphics.setColor(Color.white);
		drawImageByPixel(graphics,  width+350,18, "pictures/star.png");
		
	
		int score = data.updateScore();	
		graphics.drawString("       "+score , width+360, 60);
